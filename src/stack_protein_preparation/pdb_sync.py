...
Synchronizes a CSV file of PDB IDs with a protein data directory that stores
one subdirectory per PDB structure.
...
Meaning of 'range'
------------------
The optional 'range' column is intended to describe the residue range that
should later be used in the protein-preparation pipeline.

This module does NOT interpret or validate that range in a biological sense.
It only stores and preserves the text from the CSV.

Important limitation
--------------------
If the CSV file does not exist and is created from existing subdirectories,
the module does not know the residue range for those entries.

In that situation, it writes an empty range value:
...
- parsing the meaning of the residue range
- validating whether the range matches a sequence or PDB file
- trimming structures
- checking missing residues
- protein chemistry decisions
"""
...
def normalize_range_value(raw_range_value: str | None) -> str:
    ...
    - No further validation is performed in this module.
    """
...
def extract_pdb_id_list_from_pdb_record_list(
    ...
    This helper is useful because directory creation and PDB downloading only
    need the PDB ID, not the optional range.
    """
...
def download_pdb_file(pdb_id: str, target_pdb_file_path: Path) -> None:
    """
    Download a PDB file from the RCSB archive.
    """
...
def download_missing_pdb_files(
    ...
    """
    Download missing PDB files for a list of PDB IDs.
    """
    ...
    for pdb_id in sorted(set(pdb_id_list)):
        ...
        pdb_file_path = protein_subdirectory_path / f"{normalized_pdb_id}.pdb"

        protein_subdirectory_path.mkdir(parents=True, exist_ok=True)

        if not pdb_file_path.exists():
            print(f"[INFO] Downloading PDB for {normalized_pdb_id} -> {pdb_file_path}")
            download_pdb_file(normalized_pdb_id, pdb_file_path)
            downloaded_pdb_file_path_list.append(pdb_file_path)
        else:
            print(f"[INFO] PDB already exists for {normalized_pdb_id}: {pdb_file_path}")
...
def sync_pdb_csv_and_directories(
    ...
    Case 2:
        ...
        -> create subdirectories and download PDB files
    ...
    if csv_file_exists and not any_subdirectory_exists:
        ...
        download_missing_pdb_files(protein_data_dir, csv_pdb_id_list)
    ...
    merged_pdb_id_list = extract_pdb_id_list_from_pdb_record_list(
        merged_pdb_record_list
    )
    download_missing_pdb_files(protein_data_dir, merged_pdb_id_list)
